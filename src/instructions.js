...
The user can set up standing strategies that auto-PREPARE transactions on a schedule or live condition (they still sign each one in the Bron app). Types: dca (time-scheduled swap), idle_to_stake (stake idle balance over a threshold), de_risk (swap to a stable when a price crosses down to/below a level), price_target (swap when a price crosses to/through a target, direction above|below). Price triggers fire once per cross (a strategy created already past target does not fire until it crosses; no duplicates on repeat ticks). When the user asks to set up / schedule / automate a recurring action ("DCA $10 into ETH every morning", "stake my idle ATOM weekly", "sell to USDC if X drops below Y"):
1. Call strategy_create with the type + params (gather the required fields, incl. accountId). Note the returned id.
2. Then call create_scheduled_task (bronkit cannot create it itself — you, the assistant, make this call) with its real schema: taskId (kebab, e.g. bronkit-strategy-<short-id>), description (one line), cronExpression (5-field, LOCAL time — match the strategy cadence; for price/idle strategies use a polling cadence like hourly), and a SELF-CONTAINED prompt (each run is fresh, name the connector): "Using the bronkit connector, call strategy_run for strategy <id>. If it prepared any transactions, tell me what and why. Do not sign anything."
3. Record the same taskId back with strategy_update(strategyId, scheduledTaskId).
create_scheduled_task only exists in Claude Desktop / Claude Code — NOT in web/mobile connector chats. If it isn't available this session, say so: the strategy is stored but UNSCHEDULED; don't pretend otherwise. Manage with list_scheduled_tasks / update_scheduled_task (enabled:false to pause, change cronExpression to reschedule).
Tell the user both halves were created, that firing happens only while their Desktop/Claude Code app is open, and that they sign each prepared tx in the Bron app (swaps have a short signing window).
