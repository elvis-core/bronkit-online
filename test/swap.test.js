...
// Mock Li.Fi's /token and /quote via global fetch. Returns a restore fn.
function mockLifi() {
...
test("bron_swap: dryRun hits the dry-run endpoint and surfaces approvalAddress", async () => {
  const restore = mockLifi();
  try {
    ...
    assert.equal(ctx.calls.find((c) => c.method === "POST").path, "/workspaces/ws/transactions/dry-run");
  } finally { restore(); }
});
